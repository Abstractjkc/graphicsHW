...
void GameApp::DrawScene()
{
	...
	ResetMesh(mirrorMeshData);
	UINT stride = sizeof(VertexPosNormalTex);
	UINT offset = 0;							// 起始偏移量
	m_pd3dImmediateContext->IASetVertexBuffers(0, 1, m_pVertexBuffer.GetAddressOf(), &stride, &offset);
	// 设置默认渲染状态
	m_BasicEffect.SetMaterial(mirrorMaterial);
	m_BasicEffect.SetWorldMatrix(mirrorWorldMatrix);
	m_BasicEffect.SetRenderMirror(m_pd3dImmediateContext.Get());
	m_BasicEffect.Apply(m_pd3dImmediateContext.Get());
	m_pd3dImmediateContext->DrawIndexed(m_IndexCount, 0, 0);

	m_BasicEffect.SetDrawingStates(true);
	...
	ResetMesh(charMeshData);
	stride = sizeof(VertexPosNormalColor);
	offset = 0;							// 起始偏移量
	m_pd3dImmediateContext->IASetVertexBuffers(0, 1, m_pVertexBuffer.GetAddressOf(), &stride, &offset);
	// 设置默认渲染状态
	m_BasicEffect.SetMaterial(charMaterial);
	...
	ResetMesh(planeMeshData);
	stride = sizeof(VertexPosNormalTex);
	offset = 0;							// 起始偏移量
	m_pd3dImmediateContext->IASetVertexBuffers(0, 1, m_pVertexBuffer.GetAddressOf(), &stride, &offset);
	// 设置默认渲染状态
	m_BasicEffect.SetMaterial(mirrorMaterial);
	m_BasicEffect.SetWorldMatrix(planeWorldMatrix);
	m_BasicEffect.SetRenderPlane(m_pd3dImmediateContext.Get());
	m_BasicEffect.Apply(m_pd3dImmediateContext.Get());
	m_pd3dImmediateContext->DrawIndexed(m_IndexCount, 0, 0);
	...
	m_BasicEffect.SetDrawingStates(false);
...
bool GameApp::InitResource()
{
	...
	ResetMesh(meshData);*/
	charMeshData = Geometry::CreateChar<VertexPosNormalColor>();
	planeMeshData = Geometry::CreatePlane(XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT2(30.0f, 30.0f));
	mirrorMeshData = Geometry::CreatePlane(XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT2(100.0f, 40.0f));
	
	DirectX::XMMATRIX mTranslatePlane = XMMatrixTranslation(30.0f, 10.0f, 0.0f);
	DirectX::XMMATRIX mRotatePlane = XMMatrixRotationY(3.14f / 2.0f) * XMMatrixRotationZ(3.14f / 2.0f);
	planeWorldMatrix = mRotatePlane * mTranslatePlane;
	
	mRotatePlane = XMMatrixRotationX(-3.14f/2.0f);
	mTranslatePlane = XMMatrixTranslation(30.0f, 10.0f, 30.0f);
	mirrorWorldMatrix = mRotatePlane * mTranslatePlane;
	...
	charMaterial.ambient = XMFLOAT4(0.5f, 0.5f, 0.5f, 1.0f);
	charMaterial.diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	charMaterial.specular = XMFLOAT4(0.5f, 0.5f, 0.5f, 5.0f);
	m_BasicEffect.SetMaterial(charMaterial);
	mirrorMaterial.ambient = XMFLOAT4(0.5f, 0.5f, 0.5f, 1.0f);
	mirrorMaterial.diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 0.5f);
	mirrorMaterial.specular = XMFLOAT4(0.5f, 0.5f, 0.5f, 5.0f);
	// 摄像机位置
	...
	m_BasicEffect.SetViewMatrix(XMMatrixTranslation(-viewPos.x, -viewPos.y, -viewPos.z) * XMMatrixRotationY(-viewRot.y) * XMMatrixRotationZ(-viewRot.z) * XMMatrixRotationX(-viewRot.x));
	m_BasicEffect.SetProjMatrix(XMMatrixPerspectiveFovLH(XM_PI / 3, AspectRatio(), 1.0f, 1000.0f));
	m_BasicEffect.SetReflectMatrix(XMMatrixTranspose(XMMatrixReflect(XMVectorSet(0.0f, 0.0f, -1.0f, 30.0f))));
...
void GameApp::ResetMesh(const Geometry::MeshData<vertexType>& meshData)
{
